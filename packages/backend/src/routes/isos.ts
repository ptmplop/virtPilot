...
isosRouter.post('/upload', upload.single('file'), async (req, res) => {
  ...
  try {
    ...
    const destPath = path.join(config.isosDir, originalName);
    await fsp.rename(req.file.path, destPath);
    const displayName = (req.body as Record<string, string>).name?.trim();
    if (displayName) {
      await storageService.setIsoDisplayName(originalName, displayName);
    }
...
isosRouter.post('/download', async (req, res) => {
  ...
  try {
    savedFilename = filename?.trim() || path.basename(new URL(url).pathname) || `download-${Date.now()}.iso`;
    if (!savedFilename.endsWith('.iso')) savedFilename += '.iso';
  } catch {
